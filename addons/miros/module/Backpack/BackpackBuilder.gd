...
# 背包生成器
# 指定custom_slot路径
# 指定Backpack宿主，需要为PannelContainer类型
...
export(String) var custom_slot_path setget set_custom_slot_path
...
func generate_slots(_backpack:Node):
	...
	for i in total_count:
		var slot = load(custom_slot_path).instance()
